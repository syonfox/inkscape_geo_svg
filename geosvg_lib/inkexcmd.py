...
# Nabbed from
# https://inkscape.gitlab.io/extensions/documentation/_modules/inkex/command.html#inkscape
#   https://gitlab.com/inkscape/extensions/-/raw/master/inkex/command.py?ref_type=heads
#
# thanks bro.
# also ink-ex-cmd
# inkscape extension command
...
import re
import sys
from shutil import which as find_executable_path
...
# from .elements import SvgDocumentElement // remove this to avoid needing som much code copying
...
def which(program):
    ...
    try:
        if sys.platform == "win32":
            prog = find_executable_path(program, path=os.environ["PATH"] + ";" + os.curdir)
            if prog:
                return prog
    except ImportError:
        pass
    ...
    try:
        ...
        prog = find_executable_path(program)
        if prog:
            return prog
    except ImportError:
        pass  # python2
...
def is_instance_of_svg_document_element(svg):
    # Check if the class name of the object is 'SvgDocumentElement'
    return type(svg).__name__ == 'SvgDocumentElement'
...
def write_svg(svg, *filename):
    ...
    with open(filename, "wb") as fhl:
        if is_instance_of_svg_document_element(svg):
            svg = ElementTree(svg)
...
def list_supported_formats():
    try:
        help_output = call(INKSCAPE_EXECUTABLE_NAME, '--help')
        formats_section = re.search(r'Valid output formats:.*?([a-zA-Z0-9, ]+)', help_output, re.DOTALL)
        if formats_section:
            formats = formats_section.group(1).split(',')
            return [format.strip() for format in formats]
        return []
    except ProgramRunError as e:
        # print(f"Error listing supported formats: {e}")
        return []


def batch_process_svgs(svg_files, actions, output_dir, ext='png', dpi=96):
    """
    Batch process a list of SVG files with given actions and save the output to the specified directory.
    """
    processed_files = []
    for svg_file in svg_files:
        ...
        try:
            inkscape(svg_file, export_dpi=dpi, export_filename=output_file, export_type=ext, actions=actions)
...
def get_inkscape_version():
    try:
        version_output = call(INKSCAPE_EXECUTABLE_NAME, '--version')
